...
class DHEBag(nn.Module):
    ...
    def generate_hash_params(
        ...
        print('DHE table_sizes', table_sizes)
        
        
        
        if args_dhe["dhe_varying"]:
            ...
            for table_i in range(num_tables):
                ...
                this_k = args_dhe["k"] # default

                # if num_tables == 26:
                if 1:

                    # if table_sizes[-1] == 10131227:
                    if args_dhe["mlp_dims"][-1] == 16:
    ...
    def create_mlp(
        ...
        print('DHE mlp_dims', args_dhe["mlp_dims"])
        ...
        for table_i in range(num_tables):
            ...
            table_sz = table_sizes[table_i]            
            mlp_dims   = args_dhe["mlp_dims"] # default
    ...
    def forward(
        ...
        shared_vars.times_embed_dhe_hash.append(1000*(time_encoder))
        shared_vars.times_embed_dhe_mlp.append(1000*(time_decoder))

        # return is in a list indexed by table number, same return format as apply_emb found in dlrm_s_pytorch.py
        return ly


    def forward_individual(
        self,
        table_index,
        lS_o: torch.Tensor,
        lS_i: torch.Tensor,
        ...
        for kk, sparse_index_group_batch in enumerate(lS_i):
            sparse_offset_group_batch = lS_o[kk]  # find reduction offsets of table k

            if lS_w is not None:
                per_sample_weights = lS_w[kk]
                assert (per_sample_weights.shape == sparse_index_group_batch.shape)
            else:
                per_sample_weights = None
                
            params_table_k = self.hash_params[table_index]  # load hash parameters of table k
