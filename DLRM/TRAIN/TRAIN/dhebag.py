...
#     # z = FarmHash64(  y  )
#     z = FarmHash32(  y  )
#     print('myhash', z, y, type(x), type(z))
#                 # print('DEBUG NP', outputs, otypes, otypes[0],'\n')
#     # x=x/2
#     # return x
#     return z
...
class DHEBag(nn.Module):
        ...
        for table_i in range(num_tables):
            ...
            for i in range(len(mlp_dims) - 1):
                ...
                if i < len(mlp_dims) - 2:
                    ...
                    # print('debug', args_dhe["activation"])
                    if args_dhe["activation"] == 'relu':
                        layers.append(nn.ReLU())
                    elif args_dhe["activation"] == 'sigmoid':
    ...
    def reduce(
        ...
        xr = torch.squeeze(torch.stack(x_reduced), 1)
        # print('reduce',x.shape,xr.shape)
        return xr
        ...
        # tmp = torch.cat((offsets_cpu[1:], torch.tensor([L])))
        # tmp = tmp - offsets_cpu
        # offsets_new = tmp
        # offsets_new_new = [idx.repeat(times) for idx, times in zip(torch.arange(len(offsets_new)), offsets_new)]
        # offsets_new_new = torch.cat(offsets_new_new).to(offsets.device)          
        # xr_new = torch.zeros(offsets.shape[0], x.shape[1]).type(x.dtype).to(x.device)
        # xr_new.index_add_(dim=0, index=offsets_new_new, source=x)     
        # return xr_new
    
        # print(xr.shape, x.shape, offsets.shape)
        # print('offsets', offsets)
        # print('offsets_new', offsets_new)
        # print('offsets_new_new', offsets_new_new)
        # print(L,offsets_cpu.shape,offsets_new.shape,offsets_new_new.shape)
        # print('xr',xr)
        # print('xr_new',xr_new)
        # torch.set_printoptions(profile="full")
        # print('xr diff',xr-xr_new)
        # print(torch.sum(xr-xr_new))
        # print('\n\n')
        # exit()
    ...
    def encode(
        ...
        # h = x.detach().numpy()
        # print(h.dtype)
        # h = np.vectorize( MyHash , otypes=[np.int64]) (h)
        # print('encode ', x,h,'\n')
        # x=torch.from_numpy(h)

        # device = x.device
        # xnew = x.detach().cpu()
        # xnew.apply_( lambda a : FarmHash32(str(a))  )
        # x = xnew.to(x.device)
    ...
    def decode(
        ...
        # print('xdec ', x_decoded.shape)
        # print('dhe offsets ', offsets)
        # print('offsets shape ', offsets.shape)
        # print(' num look dhe ', self.args_dhe['num_lookups'] )

        # sum = torch.sum(offsets - torch.arange(0, offsets.shape[0], dtype=torch.int32, device=offsets.device)).item() 
        # print('arange',torch.arange(0, offsets.shape[0], dtype=torch.int32))
        # print('decode sum', sum)
    
        diff = x_decoded.shape[0] - offsets.shape[0]

        if weights is not None:
            x_decoded = x_decoded * weights[:, None]
        x_decoded = (
            # self.reduce(x_decoded, offsets) if self.args_dhe['num_lookups'] > 1 else x_decoded
            # self.reduce(x_decoded, offsets) if sum >= 1 else x_decoded
            self.reduce(x_decoded, offsets) if diff >= 1 else x_decoded
        )  # Don't use reduce logic if one-hot lookup
