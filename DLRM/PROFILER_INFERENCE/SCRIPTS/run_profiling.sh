...
do
    ...
    do
      echo "Table Size $i "
          ...
          CUSTOM_THREAD_COUNT=$t   TABLE_SIZE_ROWS=$i taskset -c $core-$(( $core+$i-1 ))    gramine-sgx  ./pytorch   run_dlrm_e64.py --inference --device 'cpu'        | tee RESULTS_OUTPUT/sgx_out_dlrm_e64__threads_$t\_tableSize_$i.log
          CUSTOM_THREAD_COUNT=$t   TABLE_SIZE_ROWS=$i taskset -c $core-$(( $core+$i-1 ))    gramine-sgx  ./pytorch   run_dlrm_e16.py --inference --device 'cpu'        | tee RESULTS_OUTPUT/sgx_out_dlrm_e16__threads_$t\_tableSize_$i.log
