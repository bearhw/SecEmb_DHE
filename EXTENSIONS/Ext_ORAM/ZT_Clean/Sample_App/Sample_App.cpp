...
int main(int argc, char *argv[]) {
  ...
  if(bulk_batch_size==0) {
    ...
    /*Test to input one block and read same block*/
    #ifdef SMALL_TEST
      unsigned char *serialized_request2 = (unsigned char*) malloc (1+ID_SIZE_IN_BYTES+DATA_SIZE);
      unsigned char* sample_data_out2 = (unsigned char*) malloc (DATA_SIZE + 1);
      unsigned char* x = (unsigned char *) malloc(64 * sizeof(uint32_t) );
      for (int i=0; i<200; i++){
        x[0] = i;
        serializeRequest(i, 'w', x, 64, serialized_request);
        ZT_Access(0, ORAM_TYPE, serialized_request, sample_data_out, request_size, response_size);
        printf("Write done: %d\n", i);
      }

      for (int j=0; j<200; j++){
        serializeRequest(j, 'r', sample_data_out2, 64, serialized_request2);
        ZT_Access(0, ORAM_TYPE, serialized_request2, sample_data_out2, request_size, response_size);
        printf("Read: %d\n", sample_data_out2[0]);
      }
      free(serialized_request2);
    #endif
    ...
    for(i=0;i<REQUEST_LENGTH;i++) {
      ...
      // encryptRequest(0, 'r', sample_data_in, DATA_SIZE, encrypted_request, tag_in, encrypted_request_size);
      serializeRequest(0, 'r', sample_data_in, DATA_SIZE, serialized_request);
      generate_request_stop = clock();		

      //Process Request:
      process_request_start = clock();		
      ZT_Access(instance_id, ORAM_TYPE, serialized_request, sample_data_out, request_size, response_size);
      process_request_stop = clock();				

      //Extract Response:
      // extract_response_start = clock();
      // extractResponse(encrypted_response, tag_out, response_size, sample_data_out);
      // extract_response_stop = clock();

      sample_data_out[DATA_SIZE]='\0';
      //printf("Obtained data : %s\n", sample_data_out);

      #ifdef RESULTS_DEBUG
          printf("datasize = %d, Fetched Data :", DATA_SIZE);
          for(uint32_t j=0; j < DATA_SIZE;j++){
        printf("%c", sample_data_out[j]);
          }
          printf("\n");
      #endif
    ...
    unreachable("Bulk does not work");
    response_size = DATA_SIZE * bulk_batch_size;
    sample_data_out = (unsigned char*) malloc (response_size);
    ...
    for(i=0;i<REQUEST_LENGTH/bulk_batch_size;i++) {
      ...
      //Process Request:
      process_request_start = clock();		
      //ZT_Bulk_Read(instance_id, ORAM_TYPE, bulk_batch_size, serialized_request, sample_data_out, encrypted_request_size, response_size);
      process_request_stop = clock();				

      //Extract Response:
      extract_response_start = clock();
      //extractResponse(encrypted_response, tag_out, response_size, sample_data_out);
      // extractBulkResponse(encrypted_response, tag_out, response_size, sample_data_out);	
      extract_response_stop = clock();
  ...
  //strcpy((char *)sample_data_in, "Hello World");
  printf("Requests Fin\n");	
