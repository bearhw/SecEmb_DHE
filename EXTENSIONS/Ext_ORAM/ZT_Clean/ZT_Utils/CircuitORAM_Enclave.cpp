...
uint32_t* CircuitORAM::GenerateRandomSequence(uint32_t length, uint32_t start, uint32_t num_blocks) {
...
uint32_t* CircuitORAM::prepare_target(uint32_t leaf, unsigned char *serialized_path, uint32_t block_size, uint32_t level, uint32_t * deepest, int32_t *target_position){	
  ...
  // printPath_buffer(serialized_path_ptr, D, data_size);
  ...
  for(i = D; i >= 0 ; i--) {
    ...
    if(i==0) {
      ...
      for(k=0;k<Z;k++) {
        ...
        // printf("D: %d K: %d Target position: %d TFlag: %d\n", i, k, target_position[i], target_position_set);
...
void CircuitORAM::EvictOnceFast(uint32_t *deepest, uint32_t *target, int32_t* deepest_position, int32_t* target_position , unsigned char * serialized_path, uint32_t level, uint32_t leaf){
  ...
  for(uint32_t i = 0;i<=(dlevel);i++) {
    ...
    omove_buffer_NEW(serialized_block_write, serialized_block_hold, tdata_size + ADDITIONAL_METADATA_SIZE, flag_1);
    ...
    if(i==0) {
      ...
      for(uint32_t k = 0; k< stash_size; k++) {
        ...
        #ifdef PROTECT_STASH
          omove_buffer_NEW(serialized_block_hold, stash_block, tdata_size + ADDITIONAL_METADATA_SIZE, flag_hold);
        #else
          omove_serialized_block(serialized_block_hold, stash_block, tdata_size, flag_hold);
        #endif
        #ifdef PROTECT_STASH
          oset_block_as_dummy_NEW(stash_block, gN, flag_hold);
        #else
      ...
      for(uint32_t k = 0;k < Z; k++){
        ...
        omove_buffer_NEW(serialized_block_hold, serialized_path_ptr, tdata_size + ADDITIONAL_METADATA_SIZE, flag_hold);
        #ifdef DEBUG_EFO
          if(flag_hold)							
        ...
        oset_block_as_dummy_NEW(serialized_path_ptr, gN, flag_hold);
        oset_value((uint32_t*) &dest, target[i], flag_hold);	
        oset_value((uint32_t*) &hold, target[i], flag_hold);							
    ...
    if(i!=0) {
      ...
      for(int32_t k = (Z-1);k >= 0; k--){
        ...
        omove_buffer_NEW(serialized_path_ptr2, serialized_block_write, tdata_size + ADDITIONAL_METADATA_SIZE, flag_w);
...
void CircuitORAM::CircuitORAM_FetchBlock(uint32_t *return_value, uint32_t leaf, uint32_t newleaf, char opType,
  ...
  for(i=0;i < ( Z * (dlevel) ); i++) {
    if(oblivious_flag) {
      ...
      omove_buffer_NEW(serialized_result_block, decrypted_path_ptr, data_size + ADDITIONAL_METADATA_SIZE, block_id==id);
      oset_block_as_dummy_NEW(decrypted_path_ptr, gN, block_id==id);
  ...
  *return_value = nextLeaf;
  ...
  //     if(block_id != gN)
  //       printf("Stash ID2: %d Level: %d\n", block_id, level);
  //     bool flag_move = (block_id == id && !isBlockDummy(stash_block, gN));
  //     if(flag_move){
  //       printf("ID found: %d Level: %d\n", block_id, level);
  //     }
  //     // omove_serialized_block(serialized_result_block, stash_block, data_size, flag_move);
  //     omove_buffer_NEW(serialized_result_block, stash_block, data_size + ADDITIONAL_METADATA_SIZE, flag_move);
  //     listptr_t=listptr_t->next;
  //   }
  //   if(level!= recursion_levels-1){
  ...
  //       oset_block_as_dummy_NEW(stash_block, gN, 1);
  //     }
  //     listptr_t=listptr_t->next;
  //   }
  ...
  // #ifdef PROTECT_STASH
  //   unsigned char *curr_nonce = serialized_result_block;
  //   uint64_t *curr64 = ((uint64_t*)(serialized_result_block));
  //   unsigned char *diff_nonce = (unsigned char*)malloc(NONCE_LENGTH);
  ...
  //   oaccess_stash_data(data_out, data_ptr, curr_nonce, tdata_size, flag_r);
  //   oaccess_stash_data(data_ptr, data_in, curr_nonce, tdata_size, flag_w);

  //   xordata64(serialized_result_block, diff_nonce, tblock_size);


  // #else
  //   #ifdef INLINE_ASM
  //     omove_buffer_NEW((unsigned char*) data_ptr, data_in, tdata_size, flag_w);
  //   #else
  //     omove_buffer((unsigned char*) data_ptr, data_in, tdata_size, flag_w);
  //   #endif

  //   #ifdef INLINE_ASM
  //     omove_buffer_NEW(data_out, (unsigned char*) data_ptr, tdata_size, flag_r);
  //   #else
  ...
  // nodev2 *listptr_t2 = recursive_stash[level].getStart();
  // printf("Start of stash2: %d %d %d\n", getId(listptr_t2->serialized_block), getTreeLabel(listptr_t2->serialized_block), isBlockDummy(serialized_result_block,gN));	



  // listptr_t2 = recursive_stash[level].getStart();
  // printf("Start of stash3: %d %d\n", getId(listptr_t2->serialized_block), getTreeLabel(listptr_t2->serialized_block));	

  // if(level == recursion_levels){
  //   #ifdef PROTECT_STASH
  //   printf("About to perform access\n");
  //     recursive_stash[level].PerformAccessOperation(opType, id, newleaf, data_in, data_out);
  //   #else
  //     recursive_stash[level].PerformAccessOperation(opType, id, newleaf, data_in, data_out);
  //   #endif
...
uint32_t CircuitORAM::access_oram_level(char opType, uint32_t leaf, uint32_t id, uint32_t position_in_id, uint32_t level, uint32_t newleaf,uint32_t newleaf_nextleaf, unsigned char *data_in,  unsigned char *data_out)
{
  ...
  // if(level==1 && leaf<2048){
  //   printPath(leaf, level);
  // }
  // printf("Downloading path for leaf: %d Level: %d\n", leaf, level);
  decrypted_path = downloadPath(leaf, level);

  // printf("Path downloaded\n");

  // return_value = CircuitORAM_Access(opType, id, position_in_id, leaf, newleaf, newleaf_nextleaf,decrypted_path, path_hash,level, data_in, data_out); 
  return_value = CircuitORAM_Access(opType, id, position_in_id, leaf, newleaf, newleaf_nextleaf,decrypted_path, level, data_in, data_out); 
    return return_value;		
}
...
void CircuitORAM::EvictionRoutine(uint32_t leaf, uint32_t level) {
  ...
  if(recursion_levels==1||level==recursion_levels-1) {
    ...
    uint32_t stash_oc;
    stash_oc = recursive_stash[level].stashOccupancy();
    printf("Level : %d, In Eviction Routine:%d\n",level,stash_oc);		
  #endif
    ...
    // printf("Leaf_left: %d | Leaf_right: %d\n", leaf_left, leaf_right);
  #else
  ...
  eviction_path_left = downloadPath(leaf_left, level);
  // printPath_buffer(eviction_path_left, dlevel, data_size);
  ...
  // printPath_buffer(eviction_path_left, dlevel, data_size);
  EvictOnceFast(deepest, target, deepest_position, target_position , eviction_path_left, level, leaf_left);
  ...
  // printf("Before upload left:\n");
  // printPath(20480, level);
  ...
  uint32_t new_path_hash_size = HASH_LENGTH*D_level[level];
      ...
      uploadPath(leaf_left, encrypted_path, path_size, level);			
    #else
      // uploadPath(leaf_left, eviction_path_left, path_size, new_path_hash, new_path_hash_size, level);
      uploadPath(leaf_left, eviction_path_left, path_size, level);
    #endif
  #else
    uploadPath(leaf_left, eviction_path_left, path_size, level);
  #endif

  // printf("After upload left:\n");
  // printPath(20480, level);
  ...
  eviction_path_right = downloadPath(leaf_right, level);
  ...
  EvictOnceFast(deepest, target, deepest_position, target_position , eviction_path_right, level, leaf_right);
  #ifdef ACCESS_DEBUG			
  ...
  */
  ...
  // printf("Before upload right:\n");
  // printPath(20480, level);	
  // printPath_buffer(eviction_path_right, dlevel, data_size);

  #ifndef PROTECT_STASH
    #ifdef ENCRYPTION_ON
      // uploadPath(leaf_right, encrypted_path, path_size, new_path_hash, new_path_hash_size, level);
      uploadPath(leaf_right, encrypted_path, path_size, level);
    #else
      // uploadPath(leaf_right, eviction_path_right, path_size, new_path_hash, new_path_hash_size, level);
      uploadPath(leaf_right, eviction_path_right, path_size, level);
    #endif
  #else
    uploadPath(leaf_right, eviction_path_right, path_size, level);
  #endif

  // printf("After upload right:\n");
  // printPath(20480, level);
  
  #ifdef SHOW_STASH_COUNT_DEBUG
      // uint32_t stash_oc;
      stash_oc = recursive_stash[level].stashOccupancy();
      printf("Level : %d , After Eviction Routine: %d\n",level,stash_oc);		
  #endif
 
}

uint32_t CircuitORAM::CircuitORAM_Access(char opType, uint32_t id, uint32_t position_in_id, uint32_t leaf, uint32_t newleaf, uint32_t newleaf_nextlevel, unsigned char* decrypted_path, uint32_t level, unsigned char* data_in, unsigned char *data_out){
...
uint32_t CircuitORAM::access(uint32_t id, int32_t position_in_id, char opType, uint8_t level, unsigned char* data_in, unsigned char* data_out, uint32_t* prev_sampled_leaf){
  ...
  if(recursion_levels ==  1) {
    ...
    CircuitORAM_Access(opType, id, -1, leaf, newleaf, -1, decrypted_path, level, data_in, data_out);
  }
    ...
    *prev_sampled_leaf = newleaf;
    ...
    // printf("0Level : %d ID: %d Fetched Leaf: %d\n", level, id, leaf);
    ...
    id_adj = id/x;
    position_in_id = id%x;
    // printf("2Calling access from level: %d ID: %d ID_adj: %d Pos: %d\n", level, id, id_adj, position_in_id);
    leaf = access(id_adj, position_in_id, opType, level-1, data_in, data_out, prev_sampled_leaf);	
    // printf("2Level : %d ID: %d ID_ADJ: %d Fetched Leaf: %d\n", level, id, id_adj, leaf);
    #ifdef ACCESS_DEBUG					
    ...
    // printf("1Calling access from level: %d ID: %d ID_Adj: %d Pos: %d\n", level, id, id_adj, nl_position_in_id);
    leaf = access(id_adj, nl_position_in_id, opType, level-1, data_in, data_out, prev_sampled_leaf);	
    // printf("1Level : %d ID: %d ID_Adj: %d Fetched Leaf: %d\n", level, id, id_adj, leaf);
