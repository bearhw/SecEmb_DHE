...
void LocalStorage::setParams(uint32_t max_blocks,uint32_t set_D, uint32_t set_Z, uint32_t stash_size, uint32_t data_size_p, bool inmem_p, uint32_t recursion_block_size_p, uint8_t recursion_levels_p)
{
  ...
  if(inmem==false) {
    unreachable("Inmem Flag is not true");
...
uint8_t LocalStorage::uploadBucket(uint32_t bucket_id, unsigned char *bucket, uint32_t bucket_size, uint8_t recursion_level)
{
  #ifdef DEBUG_LS
  ...
  if(inmem == false) {
    unreachable("inmem is false again");
    // try {
    //   #ifdef DEBUG_LS			
...
uint8_t LocalStorage::uploadPath(uint32_t leaf_label, unsigned char *path, uint8_t level, uint32_t D_level)
{
  // printf("TESTING : ID: %d, Label: %d\n", getId(path), getTreeLabel(path));
  std::string file_name_this, file_name_this_i;
  ...
  if(inmem == false) {
    ...
    */
    ...
    for(uint8_t i = 0;i<D_level;i++) {
      ...
      memcpy(inmem_tree_l[level]+((uint64_t)(Z*size_for_level))*((uint64_t)(temp-1)),path_iter,(Z*size_for_level));
      // printf("Leaf: %d\n", temp);
      // printf("LSblockID: %d treeLabel: %d\n", getId(path_iter), getTreeLabel(path_iter));
      #ifdef DEBUG_LS
...
uint8_t LocalStorage::downloadBucket(uint32_t bucket_id, unsigned char* bucket, uint32_t bucket_size , unsigned char *hash, uint32_t hash_size, uint8_t level)
{
  ...
  memcpy(bucket, inmem_tree_l[level]+((uint64_t)(Z*size_for_level))*((uint64_t)(bucket_id-1)),(Z*size_for_level));
...
unsigned char* LocalStorage::downloadPath(uint32_t leaf_label, unsigned char *path, uint8_t level, uint32_t D_lev)
{
  uint64_t pos;
  std::string file_name_this, file_name_this_i;
  uint32_t size_for_level;
  ...
  if(inmem == false) {
    ...
    // if(level==1 && temp==0){
    //   printf("Akhi: Inmemtree at 0: \n");
    //   unsigned char* bucket_iter = inmem_tree_l[1];
    //   for(uint32_t j = 0; j < Z; j++){
    //     printf("(%d,%d) :",getId(bucket_iter), getTreeLabel(bucket_iter));
    //     uint32_t no = ((size_for_level - ADDITIONAL_METADATA_SIZE))/sizeof(uint32_t);
    //     uint32_t* data_iter = (uint32_t*) (bucket_iter + ADDITIONAL_METADATA_SIZE);
    //     unsigned char *data_ptr = (unsigned char*) (bucket_iter + ADDITIONAL_METADATA_SIZE);
    ...
    //     printf("\n");
    //     bucket_iter+= size_for_level;
    //   }
    //   printf("\n");
    //   printf("\n");
    //   printf("\n");
    //   // printPath_ls(2047, 1, 13);
    // }
    ...
    for(uint8_t i = 0;i<D_lev;i++) {
      ...
      // memcpy(path_iter,inmem_tree_l[level]+((Z*size_for_level)*(temp-1)),(Z*size_for_level));
      memcpy(path_iter,inmem_tree_l[level]+((uint64_t)(Z*size_for_level))*((uint64_t)(temp-1)),(Z*size_for_level));
      // #ifdef PROTECT_POSMAP_DEBUG
        // printf("DownloadPath Id: %d | Treelabel: %d\n", getId(path_iter), getTreeLabel(path_iter));
      // #endif
...
void LocalStorage::printPath_ls(uint32_t leaf_label, uint8_t level, uint32_t D_lev)
{
  ...
  uint32_t temp = leaf_label;
  unsigned char* curr_bucket;
  uint32_t pp_blockid = 0;
  uint32_t pp_treelabel = 0;

  // for(uint8_t i = 0;i<D_lev;i++) {
  //     // memcpy(path_iter,inmem_tree_l[level]+((Z*size_for_level)*(temp-1)),(Z*size_for_level));
  //     // printf("Path iter: %d, temp: %d\n", i, temp);

      
  //     curr_bucket = inmem_tree_l[level]+((Z*size_for_level)*(temp-1));
  //     printf("Bucket Label: %d\n", temp);
  //     printf("BlockID, TreeLabel: \n");

  //     uint32_t pp_bucket_iter = 0;
  //     while(pp_bucket_iter<Z*size_for_level){
  //       pp_blockid = getId((unsigned char*) &curr_bucket[pp_bucket_iter]);
  //       // printf("GotID\n");
  //       pp_treelabel = getTreeLabel((unsigned char*) &curr_bucket[pp_bucket_iter]);
  //       // printf("Gottreelabel\n");
  //       printf("(%d, %d)\n", pp_blockid, pp_treelabel);
  //       pp_bucket_iter += size_for_level;
  //       // printf("BucketIter: %d\n", pp_bucket_iter);
  //     }
...
void LocalStorage::print_last_level_ls(uint8_t level, uint32_t start, uint32_t end){
  uint32_t size_for_level;
  ...
  for(int i=start; i<end; i++){
    curr_bucket = inmem_tree_l[level]+((Z*size_for_level)*(i-1));
    uint32_t pp_bucket_iter = 0;
      while(pp_bucket_iter<Z*size_for_level){
        pp_blockid = getId((unsigned char*) &curr_bucket[pp_bucket_iter]);
        pp_treelabel = getTreeLabel((unsigned char*) &curr_bucket[pp_bucket_iter]);
        if((pp_blockid > 50000) || (pp_treelabel > end + 1)){
          printf("FirstError Leaf: %d\n", i);
          printf("(%d, %d)\n", pp_blockid, pp_treelabel);
          loopbreak = 1;
          break;
        }
