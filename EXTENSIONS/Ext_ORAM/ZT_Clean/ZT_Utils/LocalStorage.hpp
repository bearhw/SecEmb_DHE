...
public:
  ...
  uint8_t uploadBucket(uint32_t bucket_id, unsigned char *serialized_bucket, uint32_t bucket_size, uint8_t level);
  ...
  uint8_t uploadPath(uint32_t leaf_label, unsigned char *path, uint8_t level, uint32_t D);
  unsigned char* downloadPath(uint32_t leaf_label, unsigned char *path, uint8_t level, uint32_t D);

  void printPath_ls(uint32_t leaf_label, uint8_t level, uint32_t D);
  void print_last_level_ls(uint8_t level, uint32_t start, uint32_t end);
